...
const Dashboard = () => {
  const [revenueData, setRevenueData] = useState({
    labels: ['Q1 2023', 'Q2 2023', 'Q3 2023', 'Q4 2023'], // بيانات وهمية مؤقتًا
    values: [1000000, 1200000, 1500000, 1300000], // بيانات وهمية مؤقتًا
  });
  ...
  useEffect(() => {
    const fetchFinancialData = async () => {
      try {
        ...
        console.log('Financial Data Response:', response.data); // اطبع البيانات للتأكد من شكلها

        const financialReports = response.data.data;

        // Prepare data for Revenue Trend Chart
        const revenueLabels = financialReports.map(report => `${report.period} ${report.reportYear}`);
        const revenueValues = financialReports.map(report => {
          // Assuming revenue is stored in finaldataid (you may need to adjust this logic)
          // If finaldataid is a reference, you need to populate it in the Backend
          return report.finaldataid ? 1000000 : 0; // Placeholder, replace with actual logic
        });
                  ...
                  scales: {
                    ...
                    y: {
                      ...
                      ticks: {
                        color: getStyle('--cui-body-color'),
                        maxTicksLimit: 5,
                        stepSize: Math.ceil(Math.max(...revenueData.values) / 5),
                      },
                    },
                  },
                  elements: {
